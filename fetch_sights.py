...
# Google-"type" pro Suchlauf -> unsere eigene Kategorie. Zwei getrennte
# Gruppen: "Freizeit" (Entdecken-Bereich) und "Wichtige Orte" (Rathaus,
# Polizei, Apotheken - erscheinen auf dem Stadt-Dashboard, NICHT in der
# Freizeit-Liste, damit sich Rathaus & Restaurant nicht vermischen).
LEISURE_TYPE_TO_CATEGORY = {
...
def fetch_town_places(town_key: str, town: dict, type_map: dict) -> list[dict]:
    seen_place_ids: dict[str, dict] = {}

    for place_type, category in type_map.items():
        try:
    ...
    # Beste zuerst: höhere Bewertung, dann mehr Bewertungen. Bei "wichtigen
    # Orten" (Rathaus/Polizei/Apotheke) zusätzlich nach Entfernung, da hier
    # Nähe wichtiger ist als Sternebewertung.
...
def main() -> None:
    ...
    for town_key, town in TOWNS.items():
        ...
        try:
            leisure = fetch_town_places(town_key, town, LEISURE_TYPE_TO_CATEGORY)
            print(f"  -> {len(leisure)} Freizeit-Orte gefunden.")
        except Exception as exc:  # noqa: BLE001
            print(f"[{town_key}] Fehler (Freizeit): {exc}", file=sys.stderr)
            leisure = []

        try:
            essential = fetch_town_places(town_key, town, ESSENTIAL_TYPE_TO_CATEGORY)
            print(f"  -> {len(essential)} wichtige Orte gefunden.")
        except Exception as exc:  # noqa: BLE001
            print(f"[{town_key}] Fehler (wichtige Orte): {exc}", file=sys.stderr)
            essential = []

        result[town_key] = {"leisure": leisure, "essential": essential}
